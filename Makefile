...
config.toml:
	sed 's/production/development/g; s/debug = false/debug = true/g; s/shutdownTimeout = "15s"/shutdownTimeout = "0s"/g; s/format = "json"/format = "logfmt"/g; s/level = "info"/level = "debug"/g; s/addr = ":10000"/addr = "127.0.0.1:10000"/g; s/httpAddr = ":8000"/httpAddr = "127.0.0.1:8000"/g; s/grpcAddr = ":8001"/grpcAddr = "127.0.0.1:8001"/g' config/config.toml.dist > config.toml
...
.PHONY: generate
generate: bin/swag ## Generate code
	go generate -x ./...
	swag init --parseDependency -g ./internal/app/interface/http/routes.go -o ./internal/app/interface/http/docs
